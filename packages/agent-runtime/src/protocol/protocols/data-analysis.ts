...
const commitReportedClaims = (state: DataAnalysisState, result: unknown): DataAnalysisState => {
  ...
  for (const value of claims) {
    ...
    const allManualAssertions = (requirement?.assertions ?? []).length > 0
      && (requirement?.assertions ?? []).every((assertion) => assertion.kind === "manual");
    if (allManualAssertions) {
      validateManualClaimValues(requirementId, values);
    } else {
...
/**
 * Validate claim values for manual assertions.
 *
 * Manual assertions do not produce verified values through result validation,
 * so we only check for duplicate names and skip the verified-value matching
 * that structured (metric/filter/etc.) assertions require.
 */
const validateManualClaimValues = (
  requirementId: string,
  values: AnalysisClaimValue[]
): void => {
  const submittedNames = new Set<string>();
