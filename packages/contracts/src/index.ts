...
/**
 * Input for the `create_chart` agent tool (R-015 chart visualization).
 * The agent assembles chart data from SQL results and calls this tool to
 * produce an interactive chart artifact rendered by the frontend.
 */
export type CreateChartToolInput = {
  /** Descriptive title shown in the artifact list and detail view. */
  title: string;
  /** Chart kind. The frontend can switch between bar/line/pie interactively. */
  chart_type: ChartPreviewType;
  /** Single-series data points (label + numeric value). */
  points?: ChartPreviewPoint[];
  /** Multi-series data. Takes precedence over `points` when non-empty. */
  series?: ChartPreviewSeries[];
  /** Optional unit label rendered on the value axis (e.g. "元", "%"). */
  unit?: string;
  /** Optional artifact_id of the SQL result this chart visualizes. */
