...
const schemaRowsToSummary = (
  ...
  rows.forEach((row) => {
    ...
    const tableComment = optionalRecordStringLoose(row, "table_comment") ?? optionalRecordStringLoose(row, "TABLE_COMMENT");
    if (tableComment && !table.description) {
      table.description = tableComment;
    }
    const column: SchemaSummary["tables"][number]["columns"][number] = {
    ...
    const colComment =
      optionalRecordStringLoose(row, "column_comment") ??
      optionalRecordStringLoose(row, "COLUMN_COMMENT") ??
      optionalRecordStringLoose(row, "comment") ??
      optionalRecordStringLoose(row, "description");
    if (colComment) {
