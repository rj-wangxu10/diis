...
export const createRunAgentAssembly = async (
  ...
  const semanticGraphService = new SemanticGraphService({
    ...
    dataGateway: input.dataGateway as LocalDataGateway,
    db: input.metadataStore.db
  });
  const semanticGraphClient = {
    explore: async (exploreInput: {
      user_id: string; workspace_id?: string; datasource_id: string;
      query: string; max_nodes?: number; focus?: string;
    }) => {
      const result = await semanticGraphService.explore(
        exploreInput.datasource_id,
        exploreInput.query,
        {
          ...(exploreInput.max_nodes ? { maxNodes: exploreInput.max_nodes } : {}),
          ...(exploreInput.focus ? { focus: exploreInput.focus } : {})
        }
      );
      return {
        text: result.text,
        nodes: result.nodes,
        edges: result.edges,
        snapshot_id: result.build_id
      };
    },
    hasGraph: async (hasInput: {
      user_id: string; workspace_id?: string; datasource_id: string;
    }) => {
      const builds = semanticGraphService.listBuilds(hasInput.datasource_id);
      const latest = builds.find(b => b.status === "completed");
      if (!latest) {
        return { built: false };
      }
      return {
        built: true,
        tables_count: latest.tables_count,
