...
import { EmbeddedDataLinkService } from "./embedded-datalink-service.js";
import { SemanticGraphService } from "./semantic-graph-service.js";

const EMBEDDED_DATALINK_SERVER_ID = "embedded-datalink";
const EMBEDDED_DATALINK_SERVER_NAME = "DataLink (Embedded)";
...
const handleDatasourceRequest = async (
  ...
  if (request.method === "DELETE") {
    ...
    // Delete the semantic graph (Neo4j nodes + SQLite metadata) for this datasource
    try {
      const service = getSemanticGraphService(context);
      await service.deleteGraph(id);
    } catch {
      // Non-fatal: graph deletion failure should not block datasource deletion
    }
...
const handleDatalinkRequest = async (
  ...
  // ── Semantic Graph endpoints (datasource-scoped) ───────────────────
  if (target === "datasources" && request.method === "GET") {
    // List all data sources available for graph building
    const dataSources = context.metadataStore.dataSources.list({ user_id: context.userId });
    return ok({ datasources: dataSources.map(dataSourceDto) });
  }

  if (target === "embedded-datalink" && action === "build" && request.method === "POST") {
    const body = await readJsonBody(request);
    const datasourceId = stringValue(body.datasourceId ?? body.datasource_id);
    if (!datasourceId) {
      throw new Error("DATALINK_DATASOURCE_ID_REQUIRED");
    }
    const tableWhitelist = arrayValue(body.tableWhitelist ?? body.table_whitelist);
    const service = getSemanticGraphService(context);
    const result = await service.buildGraph(
      datasourceId,
      tableWhitelist?.map((t) => String(t))
    );
    return ok({ result, server: embeddedDatalinkServerDto() });
  }

  if (target === "embedded-datalink" && action === "graph" && request.method === "GET") {
    const url = new URL(request.url ?? "", "http://localhost");
    const datasourceId = url.searchParams.get("datasourceId") ?? url.searchParams.get("datasource_id");
    if (!datasourceId) {
      throw new Error("DATALINK_DATASOURCE_ID_REQUIRED");
    }
    const service = getSemanticGraphService(context);
    const graph = await service.show(datasourceId);
    return ok({ graph, server: embeddedDatalinkServerDto() });
  }

  if (target === "embedded-datalink" && action === "explore" && request.method === "POST") {
    const body = await readJsonBody(request);
    const datasourceId = stringValue(body.datasourceId ?? body.datasource_id);
    ...
    const service = getSemanticGraphService(context);
    const result = await service.explore(datasourceId, query, {
      ...(maxNodes !== undefined ? { maxNodes: Math.floor(maxNodes) } : {}),
      ...(focus ? { focus } : {})
    });
    return ok({ result: result.text, server: embeddedDatalinkServerDto() });
  }

  if (target === "embedded-datalink" && action === "rebuild" && request.method === "POST") {
    const body = await readJsonBody(request);
    const datasourceId = stringValue(body.datasourceId ?? body.datasource_id);
    if (!datasourceId) {
      throw new Error("DATALINK_DATASOURCE_ID_REQUIRED");
    ...
    const service = getSemanticGraphService(context);
    const result = await service.rebuild(datasourceId);
    return ok({ result, server: embeddedDatalinkServerDto() });
  }

  if (target === "embedded-datalink" && action === "whitelist" && request.method === "GET") {
    const url = new URL(request.url ?? "", "http://localhost");
    const datasourceId = url.searchParams.get("datasourceId") ?? url.searchParams.get("datasource_id");
    if (!datasourceId) {
      throw new Error("DATALINK_DATASOURCE_ID_REQUIRED");
    ...
    const service = getSemanticGraphService(context);
    const tables = service.getTableWhitelist(datasourceId);
    return ok({ datasourceId, tables });
  }

  if (target === "embedded-datalink" && action === "whitelist" && request.method === "POST") {
    const body = await readJsonBody(request);
    const datasourceId = stringValue(body.datasourceId ?? body.datasource_id);
    if (!datasourceId) {
      throw new Error("DATALINK_DATASOURCE_ID_REQUIRED");
    }
    const tables = arrayValue(body.tables) ?? [];
    const service = getSemanticGraphService(context);
    service.setTableWhitelist(datasourceId, tables.map((t) => String(t)));
    return ok({ datasourceId, tables });
  }

  if (target === "embedded-datalink" && action === "builds" && request.method === "GET") {
    const url = new URL(request.url ?? "", "http://localhost");
    const datasourceId = url.searchParams.get("datasourceId") ?? url.searchParams.get("datasource_id");
    if (!datasourceId) {
      throw new Error("DATALINK_DATASOURCE_ID_REQUIRED");
    ...
    const service = getSemanticGraphService(context);
    const builds = service.listBuilds(datasourceId);
    return ok({ datasourceId, builds });
  }

  if (target === "embedded-datalink" && action === "relations" && request.method === "GET") {
    const url = new URL(request.url ?? "", "http://localhost");
    const datasourceId = url.searchParams.get("datasourceId") ?? url.searchParams.get("datasource_id");
    if (!datasourceId) {
      throw new Error("DATALINK_DATASOURCE_ID_REQUIRED");
    ...
    const service = getSemanticGraphService(context);
    const relations = service.listRelations(datasourceId);
    return ok({ datasourceId, relations });
  }

  if (target === "embedded-datalink" && action === "relations" && request.method === "POST") {
    const body = await readJsonBody(request);
    const datasourceId = stringValue(body.datasourceId ?? body.datasource_id);
    if (!datasourceId) {
      throw new Error("DATALINK_DATASOURCE_ID_REQUIRED");
    ...
    const service = getSemanticGraphService(context);
    const relationId = service.addRelation(datasourceId, {
      from_table: stringValue(body.fromTable ?? body.from_table) ?? "",
      to_table: stringValue(body.toTable ?? body.to_table) ?? "",
      from_field: stringValue(body.fromField ?? body.from_field) ?? "",
      to_field: stringValue(body.toField ?? body.to_field) ?? "",
      join_type: stringValue(body.joinType ?? body.join_type) ?? "INNER",
      cardinality: stringValue(body.cardinality) ?? "1:N",
      description: stringValue(body.description) ?? undefined
    });
    return ok({ relationId, server: embeddedDatalinkServerDto() });
  }

  if (target === "embedded-datalink" && action === "relations" && request.method === "DELETE") {
    const body = await readJsonBody(request);
    const datasourceId = stringValue(body.datasourceId ?? body.datasource_id);
    const relationId = stringValue(body.relationId ?? body.relation_id);
    if (!datasourceId || !relationId) {
      throw new Error("DATALINK_DATASOURCE_ID_AND_RELATION_ID_REQUIRED");
    }
    const service = getSemanticGraphService(context);
    const removed = service.removeRelation(datasourceId, relationId);
    return ok({ removed, server: embeddedDatalinkServerDto() });
  }

  if (target === "embedded-datalink" && action === "export" && request.method === "GET") {
    const url = new URL(request.url ?? "", "http://localhost");
    const datasourceId = url.searchParams.get("datasourceId") ?? url.searchParams.get("datasource_id");
    if (!datasourceId) {
      throw new Error("DATALINK_DATASOURCE_ID_REQUIRED");
    ...
    const service = getSemanticGraphService(context);
    const exported = await service.exportGraph(datasourceId);
    return ok({ datasourceId, ...exported });
  }

  if (target === "embedded-datalink" && action === "table-meta" && request.method === "PATCH") {
    const body = await readJsonBody(request);
    const datasourceId = stringValue(body.datasourceId ?? body.datasource_id);
    const tableName = stringValue(body.tableName ?? body.table_name);
    if (!datasourceId || !tableName) {
      throw new Error("DATALINK_DATASOURCE_ID_AND_TABLE_NAME_REQUIRED");
    }
    const service = getSemanticGraphService(context);
    await service.updateTableMeta(datasourceId, tableName, {
      ...(stringValue(body.label) !== undefined ? { label: stringValue(body.label) } : {}),
      ...(stringValue(body.description) !== undefined ? { description: stringValue(body.description) } : {})
    });
    return ok({ updated: true, server: embeddedDatalinkServerDto() });
  }

  if (target === "embedded-datalink" && action === "column-meta" && request.method === "PATCH") {
    const body = await readJsonBody(request);
    const datasourceId = stringValue(body.datasourceId ?? body.datasource_id);
    const tableName = stringValue(body.tableName ?? body.table_name);
    const columnName = stringValue(body.columnName ?? body.column_name);
    if (!datasourceId || !tableName || !columnName) {
      throw new Error("DATALINK_DATASOURCE_ID_AND_TABLE_AND_COLUMN_REQUIRED");
    }
    const service = getSemanticGraphService(context);
    await service.updateColumnMeta(datasourceId, tableName, columnName, {
      ...(stringValue(body.label) !== undefined ? { label: stringValue(body.label) } : {}),
      ...(stringValue(body.description) !== undefined ? { description: stringValue(body.description) } : {}),
      ...(stringValue(body.businessSemantic ?? body.business_semantic) !== undefined ? { businessSemantic: stringValue(body.businessSemantic ?? body.business_semantic) } : {}),
      ...(arrayValue(body.synonyms) !== undefined ? { synonyms: arrayValue(body.synonyms)?.map((s) => String(s)) } : {}),
      ...(body.isIndicator !== undefined ? { isIndicator: booleanValue(body.isIndicator, false) } : {}),
      ...(stringValue(body.indicatorCaliber ?? body.indicator_caliber) !== undefined ? { indicatorCaliber: stringValue(body.indicatorCaliber ?? body.indicator_caliber) } : {})
    });
    return ok({ updated: true, server: embeddedDatalinkServerDto() });
  }

  if (target === "embedded-datalink" && action === "graph-count" && request.method === "GET") {
    const service = getSemanticGraphService(context);
    const count = service.countBuiltGraphs();
    return ok({ count });
  }
  ...
  // ── Embedded DataLink path (legacy) ─────────────────────────────────
  // When the target is the built-in embedded server (backed by Neo4j),
  // short-circuit the external HTTP proxy and serve directly.
  if (serverId === EMBEDDED_DATALINK_SERVER_ID) {
    return handleEmbeddedDatalinkRequest(request, segments.slice(1), context);
  }
...
// ---------------------------------------------------------------------------
// Embedded DataLink (Neo4j-backed) request handler
// ---------------------------------------------------------------------------

const embeddedDatalinkServiceCache = new Map<string, EmbeddedDataLinkService>();

const getEmbeddedDatalinkService = (context: Required<ConfigApiContext>): EmbeddedDataLinkService => {
  const cacheKey = `${context.userId}:${context.workspaceId}`;
  let service = embeddedDatalinkServiceCache.get(cacheKey);
  if (!service) {
    const storageRoot = process.env.STORAGE_ROOT_DIR ?? "storage";
    service = new EmbeddedDataLinkService({
      storageRoot,
      workspaceId: context.workspaceId,
      userId: context.userId
    });
    embeddedDatalinkServiceCache.set(cacheKey, service);
  }
  return service;
};

// ---------------------------------------------------------------------------
// Semantic Graph Service (datasource-scoped, user-scoped)
// ---------------------------------------------------------------------------

const semanticGraphServiceCache = new Map<string, SemanticGraphService>();

const getSemanticGraphService = (context: Required<ConfigApiContext>): SemanticGraphService => {
  const cacheKey = `${context.userId}:${context.workspaceId}`;
  let service = semanticGraphServiceCache.get(cacheKey);
  if (!service) {
    service = new SemanticGraphService({
      userId: context.userId,
      workspaceId: context.workspaceId,
      dataGateway: context.dataGateway,
      db: context.metadataStore.db
    });
    semanticGraphServiceCache.set(cacheKey, service);
  }
  return service;
};

const embeddedDatalinkServerDto = (): Record<string, unknown> => ({
  id: EMBEDDED_DATALINK_SERVER_ID,
  name: EMBEDDED_DATALINK_SERVER_NAME,
  description: "Built-in semantic graph powered by Neo4j",
  healthStatus: "connected",
  serverUrl: "",
  apiUrl: "",
  transport: "embedded",
  toolCount: 2,
  toolNames: ["datalink_show", "datalink_explore"],
  updatedAt: new Date().toISOString()
});

const handleEmbeddedDatalinkRequest = async (
  request: IncomingMessage,
  segments: string[],
  context: Required<ConfigApiContext>
): Promise<ConfigApiResponse> => {
  const action = segments[0];
  const service = getEmbeddedDatalinkService(context);
  const serverDto = embeddedDatalinkServerDto();

  if (action === "graph" && request.method === "GET") {
    const graph = await service.show();
    return ok({ graph, server: serverDto });
  }
  if (action === "explore" && request.method === "POST") {
    const body = await readJsonBody(request);
    const query = stringValue(body.query);
    ...
    return ok({ result: result.text, server: serverDto });
  }
  if (action === "tables" && request.method === "POST") {
    const body = await readJsonBody(request);
    const source = stringValue(body.source);
    ...
    const result = await service.addTable({
      source,
      table: stringValue(body.table) ?? null,
      sourceType: stringValue(body.sourceType ?? body.source_type) ?? "csv",
      schemaName: stringValue(body.schemaName ?? body.schema_name) ?? null
    });
    return ok({ result, server: serverDto });
  }
  if (action === "tables" && request.method === "DELETE") {
    const body = await readJsonBody(request);
    const tableId = stringValue(body.tableId ?? body.table_id) ?? (segments[1] ? decodeURIComponent(segments[1]) : "");
    if (!tableId) {
      throw new Error("DATALINK_TABLE_ID_REQUIRED");
    }
    const result = await service.removeTable({
      tableId,
      cleanupOrphans: booleanValue(body.cleanupOrphans ?? body.cleanup_orphans, true)
    });
    return ok({ result, server: serverDto });
  }
  if (action === "rebuild" && request.method === "POST") {
    const body = await readJsonBody(request);
    const result = await service.rebuild(stringValue(body.mode) ?? undefined);
    return ok({ result, server: serverDto });
  }
  return methodNotAllowed();
};
...
const listDatalinkServers = (context: Required<ConfigApiContext>): Record<string, unknown>[] => {
  const external = context.metadataStore.configResources.list({
    workspace_id: context.workspaceId,
    user_id: context.userId,
    kind: "mcp-server"
  }).filter(isDatalinkServer).map(datalinkServerDto);
  // Always include the embedded Neo4j-backed server as the first option
  return [embeddedDatalinkServerDto(), ...external];
};
...
const errorResponse = (error: unknown): ConfigApiResponse => {
  ...
  // Neo4j connection errors — give a clear, actionable message
  if (
    message.includes("Neo4j") ||
    message.includes("bolt://") ||
    message.includes("Could not perform discovery") ||
    message.includes("No routing table") ||
    message.includes("ServiceUnavailable") ||
    message.includes("SessionExpired")
  ) {
    return fail(503, "NEO4J_UNAVAILABLE", `Neo4j connection failed: ${message}. Ensure Neo4j is running at ${process.env.NEO4J_URI ?? "bolt://localhost:7687"} and credentials are correct.`);
  }
