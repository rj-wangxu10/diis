...
export const createServer = async (options: CreateServerOptions = {}): Promise<Server> => {
  ...
  const server = createHttpServer(async (request, response) => {
    try {
      ...
      // Diagnostic endpoint: test Neo4j connectivity
      if (request.method === "GET" && requestUrl.pathname === "/neo4j-test") {
        const neo4jUri = process.env.NEO4J_URI ?? "bolt://localhost:7687";
        const neo4jUser = process.env.NEO4J_USER ?? "neo4j";
        const neo4jPassword = process.env.NEO4J_PASSWORD ?? "neo4j123";
        const neo4jDatabase = process.env.NEO4J_DATABASE ?? "semanticgraph";
        
        const results: Record<string, unknown> = { uri: neo4jUri, database: neo4jDatabase };
        
        // Test 1: TCP connectivity to the Bolt port
        try {
          const url = new URL(neo4jUri.replace("bolt://", "http://").replace("neo4j://", "http://"));
          const host = url.hostname;
          const port = parseInt(url.port) || 7687;
          ...
          results.tcpConnect = "OK";
        } catch (e) {
          results.tcpConnect = `FAIL: ${(e as Error).message}`;
        }
        
        // Test 2: HTTP API connectivity (port 7474)
        try {
          const httpUrl = neo4jUri.replace("bolt://", "http://").replace("neo4j://", "http://").replace(":7687", ":7474");
          const httpRes = await fetch(httpUrl, { signal: AbortSignal.timeout(5000) });
          const data = await httpRes.text();
          results.httpApi = `OK (${httpRes.status}): ${data.substring(0, 200)}`;
        } catch (e) {
          results.httpApi = `FAIL: ${(e as Error).message}`;
        }
        
        // Test 3: Neo4j driver connectivity
        try {
          const neo4jModule = await import("neo4j-driver");
          const neo4j = neo4jModule.default ?? neo4jModule;
          const driver = neo4j.driver(neo4jUri, neo4j.auth.basic(neo4jUser, neo4jPassword));
          await driver.verifyConnectivity();
          results.driverConnectivity = "OK";
          
          // Test 4: Show databases
          try {
            const session = driver.session({ database: "system" });
            const result = await session.run("SHOW DATABASES");
            results.databases = result.records.map(r => r.get("name"));
            await session.close();
          } catch (e) {
            results.showDatabases = `FAIL: ${(e as Error).message}`;
          }
          
          await driver.close();
        } catch (e) {
          results.driverConnectivity = `FAIL: ${(e as Error).message}`;
        }
