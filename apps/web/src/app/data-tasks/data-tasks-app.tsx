...
import { FinalAnswerChart } from "./components/FinalAnswerChart";
import {
...
function DataTaskWorkspace({
  ...
  const [dataLinkGraphCount, setDataLinkGraphCount] = useState(0);
  ...
  // Ensure each session has at most one datasource selected, defaulting to the
  // first usable datasource when none or multiple are enabled.
  useEffect(() => {
    if (workspaceLoading) return;
    const runnableDbs = workspaceConfig.db.filter(isConfigItemUsable);
    if (runnableDbs.length === 0) return;

    setSessions((current) => {
      let changed = false;
      const normalized = current.map((session) => {
        const enabledIds = runnableDbs
          .filter((item) => !getSessionDisabled(session).db.includes(item.id))
          .map((item) => item.id);
        const needsSelection = enabledIds.length === 0 || enabledIds.length > 1;
        if (!needsSelection) return session;
        changed = true;
        const targetId = enabledIds[0] ?? runnableDbs[0]?.id;
        return targetId ? selectSessionDatasource(workspaceConfig, session, targetId) : session;
      });
      return changed ? normalized : current;
    });
  }, [workspaceConfig, workspaceLoading]);
      ...
      setSessions((current) =>
        current.map((session) =>
          session.id === activeSessionId
            ? kind === "db"
              ? selectSessionDatasource(workspaceConfig, session, id)
              : toggleSessionResource(session, kind, id)
            : session,
        ),
      );
    },
    [activeSessionId, sessionStartedHints, sessions, workspaceConfig],
  );
  ...
  const refreshDataLinkGraphCount = useCallback(async () => {
    try {
      const res = await configApi.getSemanticGraphCount();
      setDataLinkGraphCount(res.count);
    } catch {
      setDataLinkGraphCount(0);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void refreshDataLinkGraphCount().then(() => {
      if (cancelled) return;
    });
    return () => {
      cancelled = true;
    };
  }, [capabilitiesReady, refreshDataLinkGraphCount, dataLinkPanelOpen]);
  ...
  const createSession = useCallback(() => {
    ...
    const firstDb = workspaceConfig.db.find(isConfigItemUsable);
    const seeded = firstDb
      ? selectSessionDatasource(workspaceConfig, next, firstDb.id)
      : next;
    setSessions((current) => sortChatSessions([seeded, ...current]));
  ...
  }, [clearDraftPromptRequest, defaultSessionTitle, workspaceConfig]);
  ...
  return (
    <BackendToolRuntimeProvider runtime={backendToolRuntime}>
      ...
      <SessionPane
        ...
        dataLinkGraphCount={dataLinkGraphCount}
      ...
      {dataLinkPanelOpen ? (
        <DataLinkPanel
          ...
          onGraphBuilt={refreshDataLinkGraphCount}
          onOpenMcpSettings={() => {
            setDataLinkPanelOpen(false);
            setWorkspaceFilesPanelOpen(false);
            setConfigPanel("mcp");
        ...
        <div className="flex min-h-0 min-w-0 flex-col overflow-hidden bg-surface">
          ...
          <WorkspaceConfigPanel
            ...
            onDeleteItem={async (itemId) => {
              await deleteItem(configPanel, itemId);
              if (configPanel === "db") {
                void refreshDataLinkGraphCount();
              }
            }}
  ...
  if (isFinalAnswer) {
    return (
      ...
      >
        ...
        {isFinalAnswerComplete && liveRun?.artifacts?.length ? (
          <FinalAnswerChart
            artifacts={liveRun.artifacts}
            isActive={isActive}
