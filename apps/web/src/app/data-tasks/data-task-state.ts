...
export type DataStepKind =
  ...
  | "semantic" // semantic graph exploration
...
/**
 * Single-select (radio) toggle for datasources.
 * Selecting a datasource disables all others in the same kind.
 * Requires the workspace store to enumerate all available datasource ids.
 */
export function selectSessionDatasource(
  store: WorkspaceConfigStore,
  session: ChatSession,
  id: string,
): ChatSession {
  const disabled = getSessionDisabled(session);
  // Disable every runnable datasource except the selected one.
  const allDbIds = store.db.filter(isConfigItemUsable).map((item) => item.id);
  const nextIds = allDbIds.filter((value) => value !== id);
  return {
    ...session,
    config: {
      disabled: { ...disabled, db: nextIds },
    },
  };
}
...
export function buildRunConfig(
  ...
  return {
    // Single-datasource model: only the selected datasource is enabled for the run.
    enabledDatasourceIds: activeDatasourceId ? [activeDatasourceId] : [],
