import type {
  ...
  RunCancelDto,
  RunDefaultsDto,
  SemanticGraphBuildHistoryDto,
  SemanticGraphBuildResultDto,
  SemanticGraphExportDto,
...
export const configApi = {
  ...
  // ── Semantic Graph API (datasource-scoped) ──────────────────────────

  listDatalinkDatasources(): Promise<{ datasources: DatasourceDto[] }> {
    return requestEnvelope<{ datasources: DatasourceDto[] }>(
      "/api/v1/datalink/datasources",
    );
  },
  ...
  ): Promise<{ result: SemanticGraphBuildResultDto }> {
    return requestEnvelope<{ result: SemanticGraphBuildResultDto }>(
      "/api/v1/datalink/embedded-datalink/build",
      {
        method: "POST",
        body: JSON.stringify({ datasourceId, tableWhitelist }),
      },
    );
  },

  getSemanticGraph(datasourceId: string): Promise<DatalinkGraphResponseDto> {
    const params = new URLSearchParams({ datasourceId });
    return requestEnvelope<DatalinkGraphResponseDto>(
      `/api/v1/datalink/embedded-datalink/graph?${params.toString()}`,
    );
  },

  exploreSemanticGraph(
    datasourceId: string,
    body: { focus?: string; maxNodes?: number; query: string },
  ): Promise<DatalinkToolResponseDto> {
    return requestEnvelope<DatalinkToolResponseDto>(
      "/api/v1/datalink/embedded-datalink/explore",
      {
        method: "POST",
        body: JSON.stringify({ datasourceId, ...body }),
      },
    );
  },

  rebuildSemanticGraph(datasourceId: string): Promise<{ result: SemanticGraphBuildResultDto }> {
    return requestEnvelope<{ result: SemanticGraphBuildResultDto }>(
      "/api/v1/datalink/embedded-datalink/rebuild",
      {
        method: "POST",
  ...
  getSemanticTableWhitelist(datasourceId: string): Promise<{ datasourceId: string; tables: string[] }> {
    const params = new URLSearchParams({ datasourceId });
    return requestEnvelope<{ datasourceId: string; tables: string[] }>(
      `/api/v1/datalink/embedded-datalink/whitelist?${params.toString()}`,
    );
  },

  setSemanticTableWhitelist(
    datasourceId: string,
    tables: string[],
  ): Promise<{ datasourceId: string; tables: string[] }> {
    return requestEnvelope<{ datasourceId: string; tables: string[] }>(
      "/api/v1/datalink/embedded-datalink/whitelist",
      {
        method: "POST",
        body: JSON.stringify({ datasourceId, tables }),
      },
    );
  },

  getSemanticBuildHistory(datasourceId: string): Promise<{ datasourceId: string; builds: SemanticGraphBuildHistoryDto[] }> {
    const params = new URLSearchParams({ datasourceId });
    return requestEnvelope<{ datasourceId: string; builds: SemanticGraphBuildHistoryDto[] }>(
      `/api/v1/datalink/embedded-datalink/builds?${params.toString()}`,
    );
  },

  listSemanticRelations(datasourceId: string): Promise<{ datasourceId: string; relations: SemanticGraphRelationDto[] }> {
    const params = new URLSearchParams({ datasourceId });
    return requestEnvelope<{ datasourceId: string; relations: SemanticGraphRelationDto[] }>(
      `/api/v1/datalink/embedded-datalink/relations?${params.toString()}`,
    );
  },
  ...
  addSemanticRelation(
    ...
    return requestEnvelope<{ relationId: string }>(
      "/api/v1/datalink/embedded-datalink/relations",
      {
        method: "POST",
        body: JSON.stringify({ datasourceId, ...body }),
      },
  ...
  removeSemanticRelation(datasourceId: string, relationId: string): Promise<{ removed: boolean }> {
    return requestEnvelope<{ removed: boolean }>(
      "/api/v1/datalink/embedded-datalink/relations",
      {
        method: "DELETE",
        body: JSON.stringify({ datasourceId, relationId }),
      },
    );
  },

  exportSemanticGraph(datasourceId: string): Promise<SemanticGraphExportDto> {
    const params = new URLSearchParams({ datasourceId });
    return requestEnvelope<SemanticGraphExportDto>(
      `/api/v1/datalink/embedded-datalink/export?${params.toString()}`,
    );
  },

  updateSemanticTableMeta(
    datasourceId: string,
    tableName: string,
    body: { label?: string; description?: string },
  ): Promise<{ updated: boolean }> {
    return requestEnvelope<{ updated: boolean }>(
      "/api/v1/datalink/embedded-datalink/table-meta",
      {
        method: "PATCH",
        body: JSON.stringify({ datasourceId, tableName, ...body }),
      },
    );
  },

  updateSemanticColumnMeta(
    datasourceId: string,
    tableName: string,
    columnName: string,
    body: {
    ...
    return requestEnvelope<{ updated: boolean }>(
      "/api/v1/datalink/embedded-datalink/column-meta",
      {
        method: "PATCH",
        body: JSON.stringify({ datasourceId, tableName, columnName, ...body }),
      },
    );
  },

  getSemanticGraphCount(): Promise<{ count: number }> {
    return requestEnvelope<{ count: number }>(
      "/api/v1/datalink/embedded-datalink/graph-count",
    );
  },
