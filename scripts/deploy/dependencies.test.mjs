...
test("requires Node 22, npm, python3, and neo4j", async () => {
  const entries = await inspectDependencies({
    run: runner({
      "node --version": { stdout: "v22.14.0\n" },
      "npm --version": { stdout: "10.9.0\n" },
      "python3 --version": { stdout: "Python 3.12.3\n" },
      "cypher-shell -a bolt://localhost:7687 -u neo4j -p neo4j123 RETURN 1": { stdout: "1\n" }
    })
  });
  assert.equal(entries.find((e) => e.name === "node").status, "ok");
  assert.equal(entries.find((e) => e.name === "npm").status, "ok");
  assert.equal(entries.find((e) => e.name === "python3").status, "ok");
  assert.equal(entries.find((e) => e.name === "neo4j").status, "ok");
});
...
test("non-interactive install passes --non-interactive to install-dependency.sh", async () => {
  ...
  await ensureDependencies({
    ...
    run: async (command, args = []) => {
      ...
      if (key === "python3 --version") return { stdout: "Python 3.12.3\n" };
      if (key === "cypher-shell -a bolt://localhost:7687 -u neo4j -p neo4j123 RETURN 1") return { stdout: "1\n" };
